...
import { checkoutSubscription, getShipperPlans, getShipperProfile } from '../../services/shipper/shipperService';
...
function Subscription() {
    ...
    const [shipper, setShipper] = useState<IShipper | null>(null);
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                const shipperResponse: any = await getShipperProfile();
                setShipper(shipperResponse.shipperData);
                
                // Fetch available plans regardless of subscription status
                const plansResponse: any = await getShipperPlans();
                setPlans(plansResponse.shipperPlans as IPlans[]);
                
                // If user has an active subscription, select that plan by default
                if (shipperResponse.shipper?.subscription?.planId) {
                    setSelectedPlan(shipperResponse.shipper.subscription.planId);
                }
            } catch (error) {
    ...
    const handleSubscribe = async () => {
        try {
            ...
            const response: any = await checkoutSubscription(selectedPlan);
            ...
            <div className='flex min-h-screen bg-gray-50 mt-10'>
                ...
                <div className="flex-1 w-full bg-gray-100 min-h-screen py-12 px-12">
                    <div className="max-w-6xl mx-auto">
                        {shipper?.subscription?.isActive && !showUpgrade ? (
                            ...
                            <div className="mb-12">
                                ...
                                <div className="bg-white rounded-xl shadow-lg p-8 max-w-3xl mx-auto">
                                    <div className="flex items-center justify-between mb-6">
                                        <h2 className="text-2xl font-bold text-indigo-600">{shipper.subscription.planName} Plan</h2>
                                    ...
                                    <div className="grid md:grid-cols-2 gap-8 mb-8">
                                        <div className="bg-gray-50 p-4 rounded-lg">
                                            ...
                                            <p className="text-lg font-semibold">{formatDate(shipper.subscription.startDate)}</p>
                                        </div>
                                        ...
                                        <div className="bg-gray-50 p-4 rounded-lg">
                                            ...
                                            <p className="text-lg font-semibold">{formatDate(shipper.subscription.endDate)}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="mb-8">
                                        ...
                                        {shipper.subscription.endDate && (
                                            <>
                                                <div className="w-full bg-gray-200 rounded-full h-2.5 mb-2">
                                                    <div 
                                                        className="bg-indigo-600 h-2.5 rounded-full" 
                                                        style={{ 
                                                            width: `${Math.min(100, Math.max(0, calculateDaysRemaining(shipper.subscription.endDate) / 30 * 100))}%` 
                                                        }}
                                                    ></div>
                                                </div>
                                                <p className="text-sm text-gray-600">
                                                    {calculateDaysRemaining(shipper.subscription.endDate)} days remaining
                                                </p>
                                            </>
                                        )}
                                    </div>
                            ...
                            <>
                                ...
                                <div className="flex flex-nowrap overflow-x-auto pb-6 gap-6">
                                    {plans.map((plan) => (
                                        <div
                                            ...
                                            ${shipper?.subscription?.planId === plan.id ? 'bg-indigo-50' : 'bg-white'}`}
                                        >
                                ...
                                {selectedPlan && (
                                    <div className="mt-12 text-center">
                                        ...
                                        >
                                            {shipper?.subscription?.isActive ? 'Change Subscription' : 'Subscribe Now'}
                                        </button>
