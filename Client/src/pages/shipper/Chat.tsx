...
import { fetchChats, fetchMessages, getCurrentUserId } from '../../services/shipper/shipperService';
...
const Chat: React.FC = () => {
    ...
    useEffect(() => {
        const getChatDataAndCurrentshipperId = async () => {
            try {

                const shipperId: any = await getCurrentUserId();

                if (shipperId) {
                    setCurrentUserId(shipperId.shipperId)
                }
        ...
        getChatDataAndCurrentshipperId()
    }, [])
    ...
    // useEffect(() => {
    //     if (!messages.length || !currentUserId || !selectedUser) return;
    ...
    const handleSendMessage = () => {
        ...
        //     chatId: selectedUser._id,
        //     senderId: selectedUser.shipperId,
        //     receiverId: selectedUser.transporterId._id,
        ...
        const receiverId =
            selectedUser.transporterId?._id;
                    ...
                    <div className="flex-1 overflow-y-auto">
                        {chats.map((user, i) => (
                            <div
                                ...
                                className={`flex items-center p-4 hover:bg-gray-50 cursor-pointer transition-colors ${selectedUser?.transporterId._id === user.transporterId._id ? 'bg-blue-50 border-r-2 border-blue-500' : ''
                                    }`}
                            >
                                <div className="relative">
                                    <img
                                        src={user.transporterId.profileImage}
                                        alt={user.transporterId.transporterName}
                                        className="w-12 h-12 rounded-full object-cover"
                                    />
                                    {onlineUsers.includes(user?.transporterId._id) && (
                                        <div className="absolute bottom-0 right-0 w-3 h-3 bg-green-500 rounded-full border-2 border-white"></div>
                                    )}
                                </div>
                                <div className="ml-3 flex-1 min-w-0">
                                    <div className="flex justify-between items-start">
                                        <h3 className="font-medium text-gray-900 truncate">{user.transporterId.transporterName}</h3>
                        ...
                        <>
                            ...
                            <div className="bg-white border-b border-gray-200 p-4 flex items-center justify-between">
                                <div className="flex items-center">
                                    ...
                                    <img
                                        src={selectedUser.transporterId.profileImage}
                                        alt={selectedUser.transporterId.transporterName}
                                        className="w-10 h-10 rounded-full object-cover"
                                    />
                                    <div className="ml-3">
                                        <h3 className="font-medium text-gray-900">{selectedUser.transporterId.transporterName}</h3>
                                        <p className="text-sm text-gray-500">

                                            {onlineUsers.includes(selectedUser.transporterId._id) ? 'Online' : 'Offline'}
