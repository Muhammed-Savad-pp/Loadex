...
// import { deleteNotification, fecthShipperNotifications, updateNotificationAsRead } from '../../services/shipper/shipperService';
...
// const ShipperNotification: React.FC = () => {
...
//         const response = await fecthShipperNotifications(filter);
...
import React, { useEffect, useState } from 'react';
import { Bell, Clock, CheckCheck, Trash2, Package, Truck, AlertCircle } from 'lucide-react';
import { deleteNotification, fecthShipperNotifications, updateNotificationAsRead } from '../../services/shipper/shipperService';
...
const ShipperNotification: React.FC = () => {
  ...
  const getNotificationIcon = (title: string) => {
    ...
    if (lowerTitle.includes('booking') || lowerTitle.includes('order')) {
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-green-100 text-sm">{notificationList.length} total notifications</p>
            </div>
          </div>
          {unreadCount > 0 && (
            ...
            {filteredNotifications.map(notification => (
              <div
                ...
                className={`group relative p-4 hover:bg-gray-50 transition-colors duration-200 ${
                  !notification.isRead ? 'bg-green-50/50 border-l-4 border-l-green-500' : ''
                }`}
              >
