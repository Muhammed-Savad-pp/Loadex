...
import { checkoutSession } from "../../services/shipper/shipperService";
import toast from "react-hot-toast";
import Footer from "../../components/Common/footer/Footer";
import { loadStripe } from "@stripe/stripe-js";
const stripePublicKey = import.meta.env.VITE_STRIPE_PUBLISHEBLE_KEY;
import { fetchBidsForShipper, updateBidStatusByShipper } from "../../services/bid/bidApi";
...
const ShipperBids: React.FC = () => {
    ...
    useEffect(() => {
        const findBids = async () => {
            try {
                setLoading(true);
                const response: any = await fetchBidsForShipper(page, limit, activeFilter);
    ...
    const handleStatus = async (bidId: string, status: string) => {
        try {
            if (status == 'accepted') {
                ...
                const response: any = await updateBidStatusByShipper(bidId, status);
                if (response.success) {
